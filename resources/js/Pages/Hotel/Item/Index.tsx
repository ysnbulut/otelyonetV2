...
import ItemCard from '@/Pages/Hotel/Item/components/ItemCard'
...
function Index(props: PageProps) {
	...
	const [filter, setFilter] = useState({
		...
		per_page: props.items.per_page || 12,
	})
	...
	const handleKeyDown = (e: any): void => {
		if (e.key === 'Enter') {
			router.get(route('hotel.items.index'), pickBy(filter), {
				replace: true,
				preserveState: true,
			})
		}
	...
	const handlePerPage = (e: any): void => {
		...
		router.get(route('hotel.items.index'), pickBy({...filter, per_page: e.target.value}), {
			replace: true,
			preserveState: true,
		})
	}
		...
		<>
			...
			<div className="my-2 grid grid-cols-12 gap-6">
				<div className="intro-y col-span-12 mt-2 flex flex-col items-stretch gap-2 lg:flex-row lg:items-center lg:justify-between">
					...
					<div className="flex flex-col justify-end gap-2 lg:flex-row">
						<div className="flex justify-end gap-2">
							...
							<Tippy
								as={Button}
								onClick={() => router.visit(route('hotel.items.create'))}
					...
					<div className="box col-span-12 flex min-h-96 flex-col items-center justify-center text-slate-700 text-opacity-30 dark:text-slate-500 dark:text-opacity-30">
						<div className="flex items-center justify-center gap-5">
							<Lucide
								...
								className="h-12 w-12"
							/>
							<span className="text-3xl font-semibold">Henüz bir ürün oluşturulmamış.</span>
						</div>
					</div>
				)}
				<div className="intro-y col-span-12 flex flex-col lg:flex-row">
					<Pagination className="w-full flex-shrink">
						<Pagination.Link href={props.items.first_page_url}>
							<Lucide
								icon="ChevronsLeft"
								className="h-4 w-4"
							/>
						...
						<Pagination.Link
							href={props.items.prev_page_url !== null ? props.items.prev_page_url : '#'}
							preserveScroll>
							<Lucide
								icon="ChevronLeft"
								className="h-4 w-4"
						...
						{props.items.links.map((link, key) => {
							if (key > 0 && key < props.items.links.length - 1) {
								if (props.items.current_page - 2 <= key && key <= props.items.current_page + 2) {
									return (
										<Pagination.Link
											key={key}
						...
						<Pagination.Link href={'#'}>...</Pagination.Link>
						<Pagination.Link href={props.items.next_page_url !== null ? props.items.next_page_url : '#'}>
							<Lucide
								icon="ChevronRight"
								className="h-4 w-4"
							/>
						</Pagination.Link>
						<Pagination.Link href={props.items.last_page_url}>
							<Lucide
								icon="ChevronsRight"
								className="h-4 w-4"
							/>
				...
				<div className="col-span-12 -mt-3 flex items-center justify-center text-slate-300 dark:text-darkmode-300">
					{`${props.items.total} kayıttan ${props.items.from !== null ? props.items.from : '0'} ile ${
						props.items.to !== null ? props.items.to : '0'
					} arası gösteriliyor`}
				</div>
			</div>
		</>
