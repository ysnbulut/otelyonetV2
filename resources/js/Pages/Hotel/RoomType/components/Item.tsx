...
function Item(props: ItemProps) {
	...
	const handleDeleteButtonClicked = () => {
		axios
			.delete(route('hotel.room_types.destroy', props.roomType.id))
			.then((response) => {
				props.setRoomTypes((prevState) => {
					const newState = [...prevState]
					const index = newState.findIndex((roomType) => roomType.id === props.roomType.id)
					newState.splice(index, 1)
					return newState
				})
				Toast.fire({
					icon: 'success',
					title: `Oda türü ${props.roomType.name} başarıyla silindi`,
				})
			})
			.catch((error) => {
				...
				{canBeDeleted ? (
					<>
						...
						<div className="flex flex-1 flex-col items-start justify-center">
							...
							{props.roomType?.warning_message !== null && props.roomType.warning_message !== undefined && (
								<span
									className="text-xs text-danger/60"
									dangerouslySetInnerHTML={{__html: props.roomType.warning_message}}
								/>
							)}
						</div>
					</>
					...
					<div>
						<h3 className="truncate text-xl font-semibold">
							...
							<span className="pl-2 text-xs font-thin">
								{props.roomType.adult_capacity} Yetişkin {props.roomType.child_capacity} Çocuk
							</span>
						</h3>
						<div className="mt-0.5 text-xs text-slate-500">{props.roomType.description}</div>
					</div>
				)}
				{canBeDeleted ? (
