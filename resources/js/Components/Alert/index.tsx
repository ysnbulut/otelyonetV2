...
type AlertComponent = <C extends React.ElementType = 'div'>(props: AlertProps<C>) => React.ReactElement | null

// @ts-ignore
const Alert: AlertComponent = forwardRef(
	<C extends React.ElementType>({as, dismissible, variant, ...props}: AlertProps<C>, ref?: PolymorphicRef<C>) => {
...
const DismissButton = <C extends React.ElementType = 'button'>({as, children, ...props}: DismissButtonProps<C>) => {
	...
	return (
		<Component
			...
			type="button"
			aria-label="Close"
			className={twMerge(['absolute right-0 my-auto mr-2 px-3 py-2 text-slate-800', props.className])}>
			{children}
		</Component>
	)
}
