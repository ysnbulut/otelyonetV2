...
class simple_html_dom_node
{
	...
	function makeup()
	{
		...
		foreach ($this->attr as $key => $val) {
			...
			if ($val === true) {
				...
				$ret .=
					$key . $this->_[HDOM_INFO_SPACE][$i][1] . '=' . $this->_[HDOM_INFO_SPACE][$i][2] . $quote . $val . $quote;
			}
		}
		...
		foreach ($nodes as $node) {
			...
			if ($pass && $attributes !== '' && is_array($attributes) && !empty($attributes)) {
				foreach ($attributes as $a) {
					...
					if ($lowercase) {
						$check = $this->matcht($att_expr, strtolower($att_val), strtolower($nodeKeyValue), $att_case_sensitivity);
					} else {
						$check = $this->matcht($att_expr, $att_val, $nodeKeyValue, $att_case_sensitivity);
					}
	...
	protected function matcht($exp, $pattern, $value, $case_sensitivity)
	{
		global $debug_object;
		if (is_object($debug_object)) {
			$debug_object->debug_log_entry(1);
	...
	function restore_noise($text)
	{
		...
		while (($pos = strpos($text, '___noise___')) !== false) {
			...
			if (strlen($text) > $pos + 15) {
				$key =
					'___noise___' . $text[$pos + 11] . $text[$pos + 12] . $text[$pos + 13] . $text[$pos + 14] . $text[$pos + 15];
