...
class DashboardController extends Controller
{
	...
	public function index()
	{
		...
		$transactions = Booking::select(['id', 'customer_id', 'check_in as date', 'type' => DB::raw("'booking'")])
			->union(
				CustomerPayments::select(['id', 'customer_id', 'payment_date as date', 'type' => DB::raw("'payment'")])
			)->orderBy('date', 'desc')
			->take(10)->get()->map(function ($transaction) use ($settings) {
				$info = '';
				if ($transaction->booking === 'booking') {
					...
					$payment = CustomerPayments::where('id', $transaction->id)->first();
		...
		return Inertia::render('Hotel/Dashboard/Index', [
			...
			'today_check_in_guest_count' => Guest::whereHas('bookings', function ($query) use ($now) {
				$query->where('check_in', '=', $now);
			})->count(),
			'today_check_out_guest_count' => Guest::whereHas('bookings', function ($query) use ($now) {
				$query->where('check_out', '=', $now);
			})->count(),
			'tomorrow_check_in_guest_count' => Guest::whereHas('bookings', function ($query) use ($tomorrow) {
				$query->where('check_in', '=', $tomorrow);
			})->count(),
			'tomorrow_check_out_guest_count' => Guest::whereHas('bookings', function ($query) use ($tomorrow) {
